...
#-------------Private Route Table----------------
# 프라이빗 라우트 생성 
resource "aws_route_table" "project04_private_rt" {
	vpc_id = aws_vpc.project04_vpc.id
	tags = {
		Name = "project04_private_route_table"
	}
}
...
resource "aws_route_table_association" "project04_private_rta_2a" {
	...
	route_table_id = aws_route_table.project04_private_rt.id
}
...
resource "aws_route" "project04_private_rt_table" {
	route_table_id = aws_route_table.project04_private_rt.id
